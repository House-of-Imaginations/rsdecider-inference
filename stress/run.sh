...
# mixed/batch/overload/tokenize defaults deliberately exceed it, so their k6 thresholds fail on 529s by design.
rate() {
  case "$1" in
    ...
    tokenize) echo "${TOKENIZE_RATE:-200}" ;;
  esac
}
for s in cold hot mixed batch overload tokenize; do
